const API_URL = https://script.google.com/macros/s/AKfycbxLaYqTLmVEIgZcbFgBk9RJSW7uW9laicnVDaj-UvBezu8ZiB79bNsifb-je9UvzzNk/exec;
...
async function loadData(){
  try{
    ...
    tableBody.innerHTML = `
      <tr>
        <td colspan="10" style="padding:20px;color:red;">
          Gagal memuat data. Cek URL Apps Script / akses web app.
        </td>
      </tr>
    `;
  }
...
function applyFilters(){
  ...
  filteredData = rawData.filter(item=>{
    const matchLokasi = !lokasiVal || (item.lokasi || "").toLowerCase().includes(lokasiVal);
    const matchPemutus = !pemutusVal || (item.pemutus || "").toLowerCase().includes(pemutusVal);
